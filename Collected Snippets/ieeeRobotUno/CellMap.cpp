...
CellMap::CellMap()
{
  int rows = 24;
  int columns = 24;
  prog_uint32_t cellData; // one individual cellis a 32bit unsigned integer
  PROGMEM  prog_uint32_t  cellGridMap[columns][rows] = {}; // declare 2d array representing the course map. each element is a prog_uint32_t  - an unsigned long (4 bytes) 0 to 4,294,967,295 of the form below.
...
byte cell::getLowByte()
{
 return (byte)this.+cellData
}

void cell::getHighByte();
{
  _cellData 
}
